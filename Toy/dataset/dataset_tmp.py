...
class ZiyanDatasetSampler(Sampler):
    def __init__(self, datasets, batch_size, K, shuffle=False, drop_last=False, allow_padding=True):
        """
        ...
        while batch:
            ...
            while domain_groups:
                if len(domain_groups) < self.K and self.allow_padding:
                    ...
                    flag = self.K - len(domain_groups)
                    if flag != 0: 
                        for elem in domain_order:
                            if elem not in domain_groups:
                                domain_groups.append(elem)
                                flag -= 1
                                if flag == 0:
                                    break
                    domain_groups.sort()
                ...
                # print(f"this domain are {domain_groups}, selective sample idx are {batch}")
